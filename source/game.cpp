...
Game::Game(int x, int y, int map_w, int map_h)
...
{   
    ...
    QString map_src = ":/game_objects/map_objects/map.txt";
    QPixmap blankpix;
    ...
    for (int i = 0; i < map_h; i++) {
       ...
       for (int j = 0; j < map_w; j++) {
            ...
            switch (line[j]) {
            case '1':
                map[i][j] = new GameObject(GameObject::Wall, wallPics[0]);
    ...
    connect(timer, SIGNAL(timeout()), this , SLOT(paintWall()));
    timer->start(100);
}
...
void Game::addWallPic() {
    QString address;
    for (size_t i = 1; i < 54; i++) {
        address = ":/game_objects/map_objects/wall" + QString::number(i) + ".png";
        QPixmap pixMap(address);
        wallPics.push_back(pixMap);
    }
}
...
void Game::paintWall()
{
    ...
    colorIndex = (colorIndex + 1) % wallPics.size();

    for (int i = 0; i < map_height; i++)
        for (int j = 0; j < map_width; j++)
            if (map[i][j] != nullptr && map[i][j]->get_type() == GameObject::Wall) {
                map[i][j]->setPixmap(wallPics[colorIndex]);
            }
}
